...
export class EmissionDataService {
  ...
  /**
   * Generate dummy bar data for 2025-2027 (for Stranded Assets)
  ...
  private generateBarData(propertyType: 'single' | 'multi'): BarChartData[] {
    ...
    return [
      ...
      {
        year: '2026',
        emissions: baseEmissions - 3,
        energy: baseEnergy - 15,
        propertyType,
        category: 'projected',
        label: 'Geplant 2026'
      },
      {
        year: '2027',
        emissions: baseEmissions - 6,
        energy: baseEnergy - 25,
        propertyType,
        category: 'projected',
        label: 'Ziel 2027'
      }
  ...
  /**
   * Generate single bar data for 2025 only (for Road to Paris)
   */
  private generateSingle2025BarData(propertyType: 'single' | 'multi'): BarChartData[] {
    const baseEmissions = propertyType === 'single' ? 48 : 52;
    const baseEnergy = propertyType === 'single' ? 170 : 185;
  ...
  /**
   * Load combined chart data (both line and bar data) - for Stranded Assets
   */
  ...
  /**
   * Load chart data for Road to Paris (line data + single 2025 bar)
   */
  loadRoadToParisDataset(propertyType: 'single' | 'multi'): Observable<ChartDataset> {
    return this.loadDecarbonizationPath(propertyType).pipe(
      map(lineData => ({
        lineData,
        barData: this.generateSingle2025BarData(propertyType)
      }))
    );
  }
